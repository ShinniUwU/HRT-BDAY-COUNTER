...
const BirthdayCountdown = () => {
    ...
    const fullPresetGifs = [...presetGifs, presetGifs[presetGifs.length - 1]].slice(0, 9);
    ...
    useEffect(() => {
        ...
        const calculateDaysOnHRT = () => {
            ...
            const daysOnHRT = Math.floor((currentTime.getTime() - hrtStartDate.getTime()) / (1000 * 60 * 60 * 24));
            let monthsOnHRT = Math.floor(daysOnHRT / 30);
            const remainingDays = daysOnHRT % 30;

            if (monthsOnHRT > 0) {
                setHrtInfo(`On HRT for ${monthsOnHRT} month${monthsOnHRT === 1 ? '' : 's'}, ${remainingDays} day${remainingDays === 1 ? '' : 's'}`);
            } else {
        ...
        const calculateDaysDating = () => {
            const currentTime = new Date();
            const daysDating = Math.floor((currentTime.getTime() - datingStartDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    return (
        <div className="bg-gradient-to-br from-purple-900 to-indigo-900 text-white min-h-screen flex flex-col justify-center items-center px-6 lg:px-8">
            <div className="relative isolate px-6 pt-14 lg:px-8 max-w-screen-md mx-auto w-full">
                ...
                <div className={`${styles.container} bg-white/10 backdrop-blur-md rounded-xl shadow-2xl p-6`}>
                    <h1 className={`${styles.title} text-3xl font-bold mb-6 text-pink-300`}>Hana's Trackers</h1>
                    <div className={`${styles.bulgariaTime} bg-pink-500/20 rounded-lg p-4 mb-6`}>
                    ...
                    <div className={`${styles.countdown} grid grid-cols-2 md:grid-cols-4 gap-6 mb-8`}>
                        {[{ value: days, label: 'Days' }, { value: hours, label: 'Hours' }, { value: minutes, label: 'Minutes' }, { value: seconds, label: 'Seconds' }].map(({ value, label }) => (
                            <div key={label} className={`${styles.countdownItem} bg-pink-500/20 rounded-lg p-4`}>
                                <span className={`${styles.countdownValue} text-4xl font-bold`}>{value}</span>
                                <span className={`${styles.countdownLabel} text-sm uppercase text-pink-200`}>{label}</span>
                            </div>
                        ))}
                    </div>
