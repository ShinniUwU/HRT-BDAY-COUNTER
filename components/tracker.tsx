...
const BirthdayCountdown = () => {
  ...
  const handleGifClick = () => {
    ...
    setClickCount((prevCount) => {
      ...
      // On 5th click, show all GIFs for 2s
      if (newCount === 5) {
        setShowAllGifs(true);
        setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const calculateDaysOnHRT = () => {
      ...
      const daysOnHRT = Math.floor(
        (now.getTime() - hrtStartDate.getTime()) / (1000 * 60 * 60 * 24),
      );
      const monthsOnHRT = Math.floor(daysOnHRT / 30);
      const remainingDays = daysOnHRT % 30;

      if (monthsOnHRT > 0) {
        setHrtInfo(
          `On HRT for ${monthsOnHRT} month${monthsOnHRT === 1 ? "" : "s"}, ` +
    ...
    const calculateDaysDating = () => {
      ...
      const daysDating = Math.floor(
        (now.getTime() - datingStartDate.getTime()) / (1000 * 60 * 60 * 24),
      );
